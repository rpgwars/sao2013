...
public class SolutionMutationTest {
	...
	public void mutationTest(){
		...
		for(int i = 0; i<50; i++){
			...
			assertEquals(s1.getDecisionVector() <= 1.0 && s1.getDecisionVector() >= 0.0, true);
			assertEquals(s2.getDecisionVector() <= 1.0 && s2.getDecisionVector() >= 0.0, true);
			System.out.println("s1 " + s1.getDecisionVector());
			System.out.println("s2 " + s2.getDecisionVector());
